 ...
 * <p>It is used for adjustment of the TimeKeeper. Therefore it asynchronously reads from its socket into
 * a Message instance which parses the information.</p>
 * <p>For calculation of the difference between the server and a client we need four points in time and three
 ...
 * is used to transmit the fourth timestamp back to the server.</p><p>
...
private:
    ...
    microseconds t1_; ///< Client sent PING
    microseconds t2_; ///< Server received PING
    microseconds t3_; ///< Server sent PONG
    microseconds t4_; ///< Client received PONG
